        ...
        db.collection("users").document(user_id).collection("assets").document(id).collection(detectedObject).get().addOnCompleteListener(new OnCompleteListener<QuerySnapshot>() {
            ...
            public void onComplete(@NonNull Task<QuerySnapshot> task) {
                if (task.isSuccessful()) {
                    ...
                    Log.d(TAG, "onComplete: first element is" + list.get(0));
                    for (int i = 0; i < listSize; i++) {
                        ...
                        try {
                            ...
                            Log.d(TAG, "Inside the Welcome to the app" + qrcode_urls.toString());
                        } catch (Exception e) {
                            e.printStackTrace();
                        }
                    }
                    Log.d(TAG, "Welcome to the app" + qrcode_urls.toString());
                }
            }
        });
        Log.d(TAG, "onCreate: url srting " + qrcode_urls.toString());
    }
}
